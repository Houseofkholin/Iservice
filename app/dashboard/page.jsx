...
export default function ServicesPage() {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        setIsLoading(true)
        const data = await fetchAPI("/api/users/me/services")
        setServices(data || [])
      } catch (error) {
        console.error("Error fetching services:", error)
        toast({
          title: "Error",
          description: "Failed to load your services. Please try again later.",
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
  ...
  const handleDeleteService = async () => {
    ...
    try {
      setIsDeleting(true)
      await fetchAPI(`/api/services/${serviceToDelete}`, {
        method: "DELETE",
      })
              ...
              <CardContent className="p-4 pt-0">
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center">
                    ...
                    <span>{service.delivery_time || service.deliveryTime} days</span>
                  </div>
